 ...
 *
 * <p>To define the EXPET heuristic, first let he[j] be the weighted longest processing time
 ...
 * unscheduled jobs.
 *
 * <p>Now we can define the EXPET heuristic, h[j] for job j, as follows. Case 1: If s[j] &le; 0,
 * h[j] = ht[j]. Case 2: if s[j] &ge; k*p&#772;, h[j] = he[j]. Case 3: If 0 &lt; s[j] &le;
 * k*p&#772;*ht[j]/(ht[j]-he[j]), then h[j] = ht[j] * exp((s[j](ht[j]-he[j]))/(he[j]*k*p&#772;)).
 * Case 4: If k*p&#772;*ht[j]/(ht[j]-he[j]) &lt; s[j] &lt; k*p&#772;, then h[j] = he[j]<sup>-2</sup>
 * * (ht[j] - s[j](ht[j]-he[j])/(k*p&#772;))<sup>3</sup>. For jobs with negative slack, the EXPET
 * heuristic is equivalent to weighted shortest processing time. For jobs with slack greater than
 * some multiple k of the average processing time, EXPET is equivalent to weighted longest
 * processing time.
 *
 ...
 * always produce positive values, we must adjust the values produced by the EXPET heuristic.
