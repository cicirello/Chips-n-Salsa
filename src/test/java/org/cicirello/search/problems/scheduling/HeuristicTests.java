...
public class HeuristicTests {
	...
	public void testEXPETwlptRegion() {
		...
		for (int k = 1; k <=4; k++) {
			...
			for (int x = 0; x <= 2; x++) {
				...
				FakeEarlyTardyProblem problem = new FakeEarlyTardyProblem(p, we, wt, d);
				ExponentialEarlyTardyHeuristic h = k==1 
	...
	public void testEXPETtransitionRegion2() {
		...
		for (int i = 0; i < d.length; i++) {
			...
			double pBar = 15.0/4;
			expected[i] = -numTerm*numTerm*numTerm/(p[i]*k*k*k*pBar*pBar*pBar*we[i]*we[i]);
			expected[i] += e + 2;
		}
		FakeEarlyTardyProblem problem = new FakeEarlyTardyProblem(p, we, wt, d);
		ExponentialEarlyTardyHeuristic h = new ExponentialEarlyTardyHeuristic(problem, k);
	...
	@Test
	public void testSmallestSetupPrecompute() {
		double e = SmallestSetupPrecompute.MIN_H;
		int highS = (int)Math.ceil(1 / e)*2;
		int[][] s = {
			{0, highS, highS, highS, highS},
			{1, 1, 7, 3, 1},
	...
	@Test
	public void testWSPTSetupAdjustedPrecompute() {
		double e = WeightedShortestProcessingPlusSetupTimePrecompute.MIN_H;
		int highP = (int)Math.ceil(1 / e)*2;
		int[] w =    { 1, 1, 1, 1, 0, 0, 0, 0, 2, 2, 2, 2, 1};
		int[] p =    { 1, 2, 4, 8, 1, 2, 4, 8, 1, 2, 4, 8, highP};
		double[] expected = { 1, 0.5, 0.25, 0.125, e, e, e, e, 2, 1, 0.5, 0.25, e};
