...
/**
 * Implements the scheduling cost function known as weighted flowtime. The flowtime F[j] of job j
 * is: F[j] = C[j] - r[j], where C[j] is the time it is completed by the machine, and r[j] is its
 * release time. The weighted flowtime cost function is the weighted sum over the jobs of: w[j]F[j],
