...
/**
 * <p>Implement the IterableMutationOperator interface to define a mutation
 * operator that enables iterating systematically over the neighbors of
 * a candidate solution, like one would do in a hill climber.</p>
 *
...
public interface IterableMutationOperator<T> extends MutationOperator<T> {
	...
	/**
	 * Creates and returns a {@link MutationIterator} that can be used to
	 * systematically iterate over all of the direct neighbors (i.e., a single mutation step away) 
	 * of a candidate solution, as one might do in a hill climber.
	 *
	 * @param c The candidate solution subject to the mutation.  Calling methods of the
	 * {@link MutationIterator} that is returned changes the state of that candidate solution.  
