...
/**
 * This is a wrapper class for {@link OptimizationProblem} objects that enables scaling all cost
 * values by a positive constant. This transformation doesn't change what solution is optimal, and
 * doesn't change the topology of the search space. It simply scales the cost values. For example,
 * if you want to explore the effects of the range of the cost function on the behavior of a search
 * algorithm, you can use this class to scale the cost values of a problem whose cost function range
 * is known.
 *
...
public final class CostFunctionScaler<T extends Copyable<T>> implements OptimizationProblem<T> {
   ...
   * {@inheritDoc}
   *
   * <p>In the case of the CostFunctionScaler, the cost values are all multiplied by the scale
   * factor.
   */
  @Override
