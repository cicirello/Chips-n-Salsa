...
/**
 * Internal package class for threaded calls to reoptimize.
 *
 * @param <T> The type of object being optimized.
 * @author <a href=https://www.cicirello.org/ target=_top>Vincent A. Cicirello</a>, <a
 *     href=https://www.cicirello.org/ target=_top>https://www.cicirello.org/</a>
 */
final class CallableReoptimizerFactory<T extends Copyable<T>>
    implements Function<Metaheuristic<T>, Callable<SolutionCostPair<T>>> {

  private final int runLength;

  CallableReoptimizerFactory(int runLength) {
  ...
  public Callable<SolutionCostPair<T>> apply(Metaheuristic<T> m) {
    return new CallReoptimize((ReoptimizableMetaheuristic) m);
  }

  private final class CallReoptimize implements Callable<SolutionCostPair<T>> {

    private final ReoptimizableMetaheuristic<T> m;

    CallReoptimize(ReoptimizableMetaheuristic<T> m) {
      this.m = m;
    }

    @Override
    public SolutionCostPair<T> call() {
      return m.reoptimize(runLength);
    }
  }
}
