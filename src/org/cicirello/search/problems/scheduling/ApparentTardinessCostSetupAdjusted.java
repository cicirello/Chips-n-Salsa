...
/**
 * <p>This is an implementation of a variation of 
 * the Apparent Tardiness Cost (ATC) heuristic, with a
 * simple adjustment for setup times for problems
 * with sequence-dependent setups.  Note that this is NOT
 * the Apparent Tardiness Cost with Setups (ATCS) heuristic,
 * although that heuristic can also be found in the library.</p>
 *
 * <p>ATC is defined as:
 ...
 * current time, and s[i][j] is setup time of the job if it follows
 ...
 *
 * <p>Our simple adjustment for setup time is as follows:
 * h(j) = (w[j]/(p[j]+s[i][j])) exp( -max(0,S(j)) / (k p&#772;) ).</p> 
 *
