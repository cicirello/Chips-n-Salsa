...
/**
 * <p>This class implements truncation selection for evolutionary algorithms. 
 * In truncation selection, the proportion p &isin; [0.0, 1.0) of the population with
 * greatest fitness is determined. Selection then proceeds randomly, with each member
 * of the next generation chosen uniformly at random from among the p*N members of
 * the population with highest fitness, where N is the size of the population. For example,
 * if p=0.5, and if the population size N=100, then truncation selection will select
 * individuals uniformly at random from among the 50 population members with highest fitness.</p>
 *
 * <p>Note that in this implementation, we modify the definition slightly, without loss
 * of generality. Specifically, rather than defining the operator in terms of a proportion,
 * the constructor of this class includes a parameter k, which is the absolute number
 ...
 * for truncation selection cited as O(N lg N), mistakenly assuming that sorting the
 * population by fitness is necessary. However, it is possible to determine the k most
 * fit members of the population without a full sort in O(N) time, as we can partition
 * the population into the set of the k most-fit and the set of the N-k least fit using
 * a modification of a typical order-statistics algorithm in linear time since we don't
 * actually need a total ordering over either of those sets.</p>
 *
...
public final class TruncationSelection implements SelectionOperator {
	...
	/**
	 * Constructs a truncation selection operator that selects uniformly
	 * at random from the k most fit current members of the population.
	 *
	 * @param k The number of the most fit individuals to base selection upon. The value
	 * of k must be at least 1. However, 1 is not likely to be a good choice since this means
	 * that all offspring will be based upon the single most fit individual, which means crossover
	 * will always lead to the same children.
	 *
	...
	private boolean isMedian(PopulationFitnessVector.Integer fitnesses, int[] indexes, int other1, int other2, int check) {
		return 
			(fitnesses.getFitness(indexes[check]) >= fitnesses.getFitness(indexes[other1]) && fitnesses.getFitness(indexes[check]) <= fitnesses.getFitness(indexes[other2])) ||
			(fitnesses.getFitness(indexes[check]) <= fitnesses.getFitness(indexes[other1]) && fitnesses.getFitness(indexes[check]) >= fitnesses.getFitness(indexes[other2]));		
	}
