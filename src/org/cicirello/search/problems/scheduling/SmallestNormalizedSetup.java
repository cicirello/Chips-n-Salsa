 ...
 * The normalized setup of job j is: s[i][j] / ave<sub>k</sub>(s[k][j]),
 * where s[i][j] is the setup time for job j if it follows job i, job i is
 * the most recently scheduled job, and ave<sub>k</sub>(s[k][j]) is the average
 * setup for job j, averaged over the possible remaining predecessors.
 * We define the smallest normalized setup heuristic as: 
 * h(j) = 1 / (1 + s[i][j]/ave<sub>k</sub>(s[k][j])).</p>
 *
...
public final class SmallestNormalizedSetup extends SchedulingHeuristic {
	...
	public double h(Partial<Permutation> p, int element, IncrementalEvaluation<Permutation> incEval) {
		if (HAS_SETUPS) {
			...
			if (n == 1) return 0.5;
			double s = (p.size()==0 ? data.getSetupTime(element) 
