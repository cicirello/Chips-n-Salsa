 ...
 * Initializer.
 *
 * <p>The mapping uses a classic transformation from vector of bits to a permutation of the N
 ...
 *
 * <p>Here is an example. Consider a very small permutation length N = 6. For this length
 ...
 *
 * <ul>
 *   <li>L is initially L = [0, 1, 2, 3, 4, 5], and P is initially P = [].
 *   <li>The first 3 bits, 111, is 7 in decimal, and 7 mod 6 = 1. L[1]=1, so we remove the 1 from L
 *       and add it to the end of P. This leads to L = [0, 2, 3, 4, 5], and P is P = [1].
 *   <li>The next 3 bits, 110, is 6 in decimal, and 6 mod 5 = 1. L[1]=2, so we remove the 2 from L
 *       and add it to the end of P. This leads to L = [0, 3, 4, 5], and P is P = [1, 2].
 *   <li>The next 3 bits, 001, is 1 in decimal, and 1 mod 4 = 1. L[1]=3, so we remove the 3 from L
 *       and add it to the end of P. This leads to L = [0, 4, 5], and P is P = [1, 2, 3].
 *   <li>The next 3 bits, 010, is 2 in decimal, and 2 mod 3 = 2. L[2]=5, so we remove the 5 from L
 *       and add it to the end of P. This leads to L = [0, 4], and P is P = [1, 2, 3, 5].
 *   <li>The last 3 bits, 100, is 4 in decimal, and 4 mod 2 = 0. L[0]=0, so we remove the 0 from L
 *       and add it to the end of P. This leads to L = [4], and P is P = [1, 2, 3, 5, 0].
 *   <li>Reached the end of the bit vector, and there is only one element left in L, the 4, which we
...
public class PermutationToBitVectorProblem implements Initializer<BitVector> {
   ...
   *
   * <p>The superclass, {@link PermutationToBitVectorProblem}, handles the transformation between
