		...
		for (int n = 1; n < 5; n++) {
			for (int r = 0; r < rdd.length; r++) {
				for (int t = 0; t < tf.length; t++) {
					...
					s.toFile(out);
					WeightedStaticScheduling s2 = new WeightedStaticScheduling(new StringReader(sOut.toString()), n, 0);
		...
		for (int n = 8; n <= 512; n *= 8) {
			for (int r = 0; r < rdd.length; r++) {
				for (int t = 0; t < tf.length; t++) {
					...
					for (int x = 0; x < n; x++) {
						assertTrue(s.getProcessingTime(x) >= WeightedStaticScheduling.MIN_PROCESS_TIME
							&& s.getProcessingTime(x) <= WeightedStaticScheduling.MAX_PROCESS_TIME);
						assertTrue(s.getWeight(x) >= WeightedStaticScheduling.MIN_WEIGHT
							&& s.getWeight(x) <= WeightedStaticScheduling.MAX_WEIGHT);
						assertTrue(s.getDueDate(x) >= MIN_DUEDATE
							&& s.getDueDate(x) <= MAX_DUEDATE);
					}
					boolean diffP = false;
					for (int x = 1; x < n; x++) {
