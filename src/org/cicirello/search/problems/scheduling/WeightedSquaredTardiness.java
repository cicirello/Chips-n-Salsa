 ...
 * weighted squared tardiness.  The lateness L[j] of job j
 * is: L[j] = C[j] - d[j], where C[j] is the time 
 * it is completed by the machine, and d[j] is its
 * due date.  The tardiness of job j is: T[j] = max(0, L[j]).
 * That is, although lateness can be negative, tardiness is
 * never negative (i.e., no reward for a job completing early). 
 * The weighted squared tardiness cost function
 * is the weighted sum of the squares of the tardiness of 
 * the jobs of, i.e., the sum of w[j]T[j]<sup>2</sup> over all
 * jobs j, where w[j] is job j's weight.
 *
...
public final class WeightedSquaredTardiness implements SingleMachineSchedulingProblem {
	...
	public int cost(Permutation candidate) {
		...
		for (int i = 0; i < c.length; i++) {
			...
			if (lateness > 0) {
				total += instanceData.getWeight(i) * lateness * lateness;
			}
		}
		return total;
	}
