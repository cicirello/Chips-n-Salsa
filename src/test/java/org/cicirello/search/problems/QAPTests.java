...
public class QAPTests {
        ...
        assertThrows(
            ...
            () ->
                QuadraticAssignmentProblem.createInstance(
                    cost, new int[cost.length][cost.length + 1]));
    thrown =
        assertThrows(
            IllegalArgumentException.class,
            () ->
                QuadraticAssignmentProblem.createInstance(
                    new int[cost.length][cost.length + 1], dist));
    thrown =
        assertThrows(
            IllegalArgumentException.class,
            () ->
                QuadraticAssignmentProblem.createInstance(
                    new int[cost.length + 1][cost.length + 1], dist));
  }
  ...
  public void testQAPUniformRandomNoSeed() {
    ...
    int minD = 100;
    int maxD = 120;
    QuadraticAssignmentProblem problem =
        QuadraticAssignmentProblem.createUniformRandomInstance(5, minC, maxC, minD, maxD);
    assertEquals(0, problem.minCost());
  ...
  public void testQAPUniformRandomWithSeed() {
    ...
    int maxD = 120;
    QuadraticAssignmentProblem problem =
        QuadraticAssignmentProblem.createUniformRandomInstance(5, minC, maxC, minD, maxD, 42);
    assertEquals(0, problem.minCost());
    assertEquals(n, problem.size());
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
