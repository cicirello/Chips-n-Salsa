 ...
 * parallel with multiple threads. It specifically requires that all metaheuristics are solving the
 * same problem, but otherwise, they may be the same or different metaheuristics.
 *
...
public class ParallelMetaheuristic<T extends Copyable<T>>
  ...
  /**
   * Constructs a parallel metaheuristic that executes multiple identical copies of a metaheuristic
  ...
  /**
   * Constructs a parallel metaheuristic that executes multiple metaheuristics in parallel across
   * multiple threads.
   *
   * @param searches A collection of the metaheuristics to run in parallel. The number of threads
   *     will be equal to the size of this collection.
  ...
  /**
   * Executes a parallel metaheuristic search. The number of threads, the specific metaheuristic
   * executed by each thread, etc are determined by how the ParallelMetaheuristic was configured at
   * the time of construction. The optimize method runs the optimize method of each of the parallel
   * instances of the search for the specified run length (same run length for all parallel
   * instances), keeping track of the best solution across the multiple parallel runs of the search.
   * The meaning of run length may vary based on the component metaheuristics. Each run of each
   * parallel search begins at a new randomly generate initial state.
   *
   ...
   *     ProgressTracker}, which contains the best of all runs across all calls to optimize. Returns
  ...
  /**
   * Initiates an orderly shutdown of the thread pool used by this ParallelMetaheuristic. The
   * ParallelMetaheuristic utilizes a fixed thread pool so that multiple calls to the {@link
   * #optimize} method can reuse threads to minimize the expensive task of thread creation. When you
   * no longer need the ParallelMetaheuristic, you should call the close method to ensure that
  ...
  /**
   * Gets the total run length of all runs of all parallel instances of the underlying
   ...
   *
   * <p>The meaning of run length may vary based on the underlying metaheuristic.
   *
  ...
  final SolutionCostPair<T> threadedOptimize(
      Function<Metaheuristic<T>, Callable<SolutionCostPair<T>>> icf) {
