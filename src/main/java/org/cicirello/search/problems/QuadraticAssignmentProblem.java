...
public final class QuadraticAssignmentProblem implements IntegerCostOptimizationProblem<Permutation> {
	 ...
	 * @throws IndexOutOfBoundsException if either i or j is less than 0, or if either i or j is greater than
	 * or equal to size().
	 */
	...
	/**
	 * Creates an instance of the QAP problem with cost and distance matrices formed from
	 * uniformly random integers. Note that the diagonal is always 0.
	 *
	 ...
	 * @param minDistance Distances are uniform at random from an interval with minDistance as the minimum.
	 * @param maxDistance Distances are uniform at random from an interval with maxDistance as the maximum.
	 *
	 * @return an instance of the QAP problem with uniform random cost and distance matrices
	 *
	 * @throws IllegalArgumentException if size is less than 1, or maxCost is less than minCost, or maxDistance is less than minDistance.
	 */
	public static QuadraticAssignmentProblem createUniformRandomInstance(int size, int minCost, int maxCost, int minDistance, int maxDistance) {
		return createUniformRandomInstance(size, minCost, maxCost, minDistance, maxDistance, new SplittableRandom());
	}
