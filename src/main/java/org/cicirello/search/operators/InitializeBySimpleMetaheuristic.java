...
/**
 * <p>This class implements the {@link Initializer} interface to provide
 ...
 * generated via a metaheuristic.  For example, you can use this class
 * to generate initial solutions for simulated annealing, etc by first running
 * a hill climber to climb to a local optima first before running the simulated
 * annealer.  The InitializeBySimpleMetaheuristic class can generate initial
 * candidate solutions using an object of any class that implements the
 * {@link SimpleMetaheuristic} interface.</p>
 *
 *
...
public final class InitializeBySimpleMetaheuristic<T extends Copyable<T>> implements Initializer<T> {
	...
	/**
	 * Constructs an Initializer that creates initial candidate solutions for metaheuristics
	 * by running another simpler metaheuristic, such as a hill climber, 
	 * from a random initial solution, climbing to a local optima.
