 ...
 * apply mutation to each population member with rate M, replace.
 *
...
final class SimpleGeneration<T extends Copyable<T>> implements Generation<T> {
	...
	SimpleGeneration(MutationOperator<T> mutation, double mutationRate, CrossoverOperator<T> crossover, double crossoverRate) {
		if (mutation == null) {
