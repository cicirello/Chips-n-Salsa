 ...
 * {@link org.cicirello.search.problems.OptimizationProblem OptimizationProblem} 
 * or {@link IntegerCostOptimizationProblem}. 
 ...
 *
 * <p>This class transforms the cost of solution s to fitness 
 * with the following
 * transformation: fitness(s) = -cost(s), where cost(s)
 * refers to the {@link IntegerCostOptimizationProblem#cost} method.</p>
 *
 * <p>Because this transformation produces negative fitness values, it is not
 ...
 * operator that uses only relative fitness values. For example, {@link TournamentSelection} 
 * and other selection operators that only care if one fitness is higher or lower than another
 * will work fine with negative fitness values.</p>
 *
 ...
 * {@link FitnessProportionalSelection}, {@link StochasticUniversalSampling},
 * {@link BiasedFitnessProportionalSelection}, and {@link BiasedStochasticUniversalSampling}.
...
public final class NegativeIntegerCostFitnessFunction<T extends Copyable<T>> implements FitnessFunction.Integer<T> {
	 ...
	 * the following transformation: fitness(s) = -cost(s).
	 *
	...
	@Override
	public int fitness(T candidate) {
