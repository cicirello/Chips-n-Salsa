 ...
 */
public class ParallelReoptimizableMetaheuristic<T extends Copyable<T>>
    extends ParallelMetaheuristic<T> implements ReoptimizableMetaheuristic<T> {
   ...
   */
  public ParallelReoptimizableMetaheuristic(
   ...
   * executed by each thread, etc are determined by how the ParallelReoptimizableMetaheuristic was
   * configured at the time of construction. The reoptimize method runs the reoptimize method of
   * each of the parallel instances of the search the specified for the specified run length
   * (identical for all parallel instances), keeping track of the best solution across the multiple
   * parallel runs of the search.
   *
   ...
   * @throws IllegalStateException if the {@link ParallelMetaheuristic#close} method was previously
   *     called.
   */
  @Override
  public final SolutionCostPair<T> reoptimize(int runLength) {
    return threadedOptimize(new CallableReoptimizerFactory<T>(runLength));
  }
