...
/**
 * This class implements Cauchy mutation with support for the {@link #undo} method. Cauchy mutation
 * is for mutating floating-point values. This class can be used to mutate objects of any of the
 * classes that implement the {@link RealValued} interface, including both univariate and
 ...
 */
public class UndoableCauchyMutation<T extends RealValued> extends AbstractUndoableRealMutation<T>
    implements Copyable<UndoableCauchyMutation<T>> {
   ...
   */
  UndoableCauchyMutation(double scale, DoubleBinaryOperator transformer, Selector selector) {
  ...
  /**
   * Creates a Cauchy mutation operator with scale parameter equal to 1 that supports the undo
   * operation.
   *
   * @param <T> The specific RealValued type.
  ...
  /**
   * Creates a Cauchy mutation operator that supports the undo operation, and such that the mutate
   * method constrains each mutated real value to lie in the interval [lowerBound, upperBound].
   *
   * @param scale The scale parameter of the Cauchy.
