 ...
 * annealing known as exponential cooling (sometimes referred to as geometric cooling). In this
 * parameter-free version of the exponential cooling schedule, the initial temperature, the value of
 * alpha, and the step size are all computed by the ParameterFreeExponentialCooling object based on
 * an estimate of the cost difference between random neighbors, and the run length specified upon
 * calling the {@link #init} method.
 *
 * <p>In exponential cooling, the k-th temperature, t<sub>k</sub>, is determined as follows:
 * t<sub>k</sub> = &alpha;<sup>k</sup> * t<sub>0</sub>, where t<sub>0</sub> is the initial
 * temperature and &alpha; is the cooling rate. The new temperature is usually computed
 * incrementally from the previous with: t<sub>k</sub> = &alpha; * t<sub>k-1</sub>. In some
 ...
 * declines to 0.001 by the end of the run. Specifically, we set &alpha; = (0.001 /
 * t<sub>0</sub>)<sup>1 / ceiling(k / steps)</sup>, where k is the number of remaining iterations
 * (maxEvals reduced by the number of iterations necessary to obtain the 10 samples used to compute
 * t<sub>0</sub>) and where steps is set to the lowest power of 2 such that the &alpha; we compute
 * is &alpha; &le; 0.999. The rationale for setting steps to a power of 2 is for efficiency in
 * computing &alpha; and steps (start steps at 1 and double until &alpha; is in target range,
 * relatively few iterations necessary). The rationale for setting &alpha; &le; 0.999 is to avoid
 * any numerical issues that may arise from repeatedly multiplying by a value that is very close to
 * 1.0.
 *
