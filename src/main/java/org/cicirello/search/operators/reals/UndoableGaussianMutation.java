...
/**
 * This class implements Gaussian mutation with support for the {@link #undo} method. Gaussian
 * mutation is for mutating floating-point values. This class can be used to mutate objects of any
 * of the classes that implement the {@link RealValued} interface, including both univariate and
 ...
 * an UndoableGaussianMutation object to the {@link #mutate} method of a UndoableGaussianMutation
 * object.
 *
 * <p>To construct an UndoableGaussianMutation, you must use one of the factory methods. See the
 * various {@link #createGaussianMutation} methods.
 *
 ...
 */
public final class UndoableGaussianMutation<T extends RealValued>
    extends AbstractUndoableRealMutation<T> implements Copyable<UndoableGaussianMutation<T>> {
   ...
   */
  UndoableGaussianMutation(double sigma, RandomizedDoubleBinaryOperator transformer) {
  ...
  /**
   * Creates a Gaussian mutation operator with standard deviation equal to 1, and which supports the
   * {@link #undo} method.
   *
   * @param <T> The specific RealValued type.
  ...
  /**
   * Creates a Gaussian mutation operator, and which supports the {@link #undo} method.
   *
   * @param sigma The standard deviation of the Gaussian.
   * @param <T> The specific RealValued type.
   * @return A Gaussian mutation operator.
  ...
  public static <T extends RealValued> UndoableGaussianMutation<T> createGaussianMutation(
      double sigma) {
    return new UndoableGaussianMutation<T>(sigma, (old, param, r) -> old + r.nextGaussian(param));
  }

  /**
   * Creates a Gaussian mutation operator, such that the mutate method constrains each mutated real
   * value to lie in the interval [lowerBound, upperBound], and which supports the {@link #undo}
   * method.
   *
   * @param sigma The standard deviation of the Gaussian.
  ...
  /**
   * Create a Gaussian mutation operator, and which supports the {@link #undo} method.
   *
