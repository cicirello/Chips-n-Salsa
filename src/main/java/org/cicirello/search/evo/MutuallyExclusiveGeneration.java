...
final class MutuallyExclusiveGeneration<T extends Copyable<T>> implements Generation<T> {
	...
	MutuallyExclusiveGeneration(MutationOperator<T> mutation, double mutationRate, CrossoverOperator<T> crossover, double crossoverRate) {
		if (mutation == null) {
	...
	public int apply(Population<T> pop) {
		...
		if (crossed < LAMBDA && M_PRIME > 0.0) {
			mutateCount = M_PRIME < 1.0 ? RandomVariates.nextBinomial(LAMBDA - crossed, M_PRIME) : LAMBDA - crossed;
