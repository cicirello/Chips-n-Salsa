 ...
 * any class that implements the {@link Metaheuristic} interface. A multistart metaheuristic returns
 ...
 * initialize the search with a combination of a {@link Metaheuristic} and number of threads along
 * with either a {@link RestartSchedule} for the purpose of specifying run lengths for the restarts,
 * or a run length if all runs are to be of the same length. You can also initialize the search with
 * a Collection of {@link RestartSchedule} objects, one for each thread (with number of threads
 * implied by size of Collection. Or you can initialize the search with a Collection of {@link
 * Metaheuristic} objects and a Collection of {@link RestartSchedule} objects (both Collections of
 * the same size). You can also initialize the search with a {@link Multistarter} configured with
 * your restart schedule, along with the number of threads, or a Collection of {@link Multistarter}
 * objects.
 *
...
public class TimedParallelMultistarter<T extends Copyable<T>>
  ...
  public TimedParallelMultistarter(Metaheuristic<T> search, int runLength, int numThreads) {
    this(
        toMultistarters(
            new Multistarter<T>(search, new ConstantRestartSchedule(runLength)), numThreads),
        false);
  }
  ...
  public TimedParallelMultistarter(Metaheuristic<T> search, RestartSchedule r, int numThreads) {
    this(toMultistarters(new Multistarter<T>(search, r), numThreads), false);
  }
  ...
  public TimedParallelMultistarter(
    ...
    this(toMultistarters(search, schedules), false);
  }
  ...
  public TimedParallelMultistarter(
    ...
    this(toMultistarters(searches, schedules), false);
  }
  ...
  public TimedParallelMultistarter(Collection<? extends Metaheuristic<T>> searches, int runLength) {
    this(
        toMultistarters(
            searches, ConstantRestartSchedule.createRestartSchedules(searches.size(), runLength)),
        false);
  }
  ...
  public TimedParallelMultistarter(Multistarter<T> multistartSearch, int numThreads) {
    this(toMultistarters(multistartSearch, numThreads), false);
  }
  ...
  public TimedParallelMultistarter(Collection<? extends Multistarter<T>> multistarters) {
    this(verifyMultistarterCollection(multistarters), true);
  }

  /*
   * package private for use by subclasses in same package.
   */
  TimedParallelMultistarter(ArrayList<Multistarter<T>> multistarters, boolean verifyState) {
    this.multistarters = multistarters;
   ...
   *
   * @param timeUnit The unit of time to use with the {@link #optimize} method, specified in
   *     milliseconds. For example, if timeUnit equals 2000, then the call optimize(3) will run for
   *     approximately 6 seconds, since 3 * 2000 is 6000 milliseconds, which is 6 seconds.
   * @see #TIME_UNIT_MS
   * @see #getTimeUnit
   ...
   *
   * @return The unit of time used by the {@link #optimize} method, specified in milliseconds. For
   *     example, if timeUnit equals 2000, then the call optimize(3) will run for approximately 6
   *     seconds, since 3 * 2000 is 6000 milliseconds, which is 6 seconds.
   * @see #TIME_UNIT_MS
   * @see #setTimeUnit
   */
  ...
  /**
   * Gets a list of the best solution stored in this search's {@link ProgressTracker} at each time
   * interval of the most recent call to the {@link #optimize} method, or null if {@link #optimize}
   * has not been called. Note that the ProgressTracker stores the best solution found across all
   * calls to the {@link #optimize} method, so the solutions in the list returned by this method may
   * or may not have been found during the most recent call to {@link #optimize}. The length of the
   * list returned will be no greater than the value passed to {@link #optimize} for the time
   * parameter. The length will be less than the time parameter in the event that the search
   * terminates early due to finding the optimal solution.
   *
   * @return A list of the best found solution, as stored in the ProgressTracker, at each time
   *     interval during the most recent call to the {@link #optimize} method, or null if {@link
   ...
   * the best possible solution was found. Each restart of each parallel search begins at a new
   ...
   *     the constant {@link #TIME_UNIT_MS} unless changed by a call to the {@link #setTimeUnit}
   *     method. For example, assuming {@link #setTimeUnit} has not been called, then the search
   *     will run for approximately: time * {@link #TIME_UNIT_MS} milliseconds.
   ...
   * TimedParallelMultistarter utilizes a fixed thread pool so that multiple calls to the {@link
   * #optimize} method can reuse threads to minimize the expensive task of thread creation. When you
   * no longer need the TimedParallelMultistarter, you should call the close method to ensure that
   * unneeded threads do not persist. Once close is called, all subsequent calls to {@link
   * #optimize} will throw an exception.
   *
   * <p>This method is invoked automatically on objects managed by the try-with-resources statement.
   */
  @Override
  ...
  /**
   * Gets the total run length of all restarts of all parallel instances of the underlying
   * metaheuristics combined. This may differ from what may be expected based on run lengths passed
   * to the optimize and reoptimize methods of the underlying metaheuristics. For example, the
   * optimize method terminates if it finds the theoretical best solution, and also immediately
   * returns if a prior call found the theoretical best. In such cases, the total run length may be
   * less than the requested run length.
   *
   * <p>The meaning of run length may vary based on what metaheuristic is being restarted.
   *
  ...
  private static <T2 extends Copyable<T2>> ArrayList<Multistarter<T2>> verifyMultistarterCollection(
      Collection<? extends Multistarter<T2>> multistarters) {
    ProgressTracker<T2> t = null;
    Problem<T2> problem = null;
    for (Multistarter<T2> m : multistarters) {
      if (problem == null) {
    ...
    return new ArrayList<Multistarter<T2>>(multistarters);
  }
   ...
   * @throws IllegalArgumentException if numThreads is less than 1.
   */
  private static <T2 extends Copyable<T2>> ArrayList<Multistarter<T2>> toMultistarters(
      Multistarter<T2> multistartSearch, int numThreads) {
    if (numThreads < 1) throw new IllegalArgumentException("must be at least 1 thread");
    ArrayList<Multistarter<T2>> restarters = new ArrayList<Multistarter<T2>>();
   ...
   */
  private static <T2 extends Copyable<T2>> ArrayList<Multistarter<T2>> toMultistarters(
      Metaheuristic<T2> search, Collection<? extends RestartSchedule> schedules) {
    if (schedules.size() < 1)
      throw new IllegalArgumentException("Must pass at least one schedule.");
    ArrayList<Multistarter<T2>> restarters = new ArrayList<Multistarter<T2>>(schedules.size());
    boolean addedFirst = false;
    for (RestartSchedule r : schedules) {
      if (addedFirst) restarters.add(new Multistarter<T2>(search.split(), r));
      else {
        restarters.add(new Multistarter<T2>(search, r));
        addedFirst = true;
      }
    }
    return restarters;
  ...
  private static <T2 extends Copyable<T2>> ArrayList<Multistarter<T2>> toMultistarters(
    ...
    ArrayList<Multistarter<T2>> restarters = new ArrayList<Multistarter<T2>>(searches.size());
    Iterator<? extends RestartSchedule> rs = schedules.iterator();
    ProgressTracker<T2> t = null;
    Problem<T2> problem = null;
    for (Metaheuristic<T2> s : searches) {
      if (problem == null) {
      ...
      restarters.add(new Multistarter<T2>(s, rs.next()));
    }
    return restarters;
  }
}
