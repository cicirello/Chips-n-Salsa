...
public final class BitVector implements Copyable<BitVector> {
   ...
   *
   * @param bitLength The length of the bit vector in number of bits.
   * @param randomize if true, then the vector is initialized with random bit values; and otherwise
   *     initializes it to a vector of all 0 bits.
   * @throws IllegalArgumentException if bitLength &lt; 0.
   */
   ...
   * @throws IllegalArgumentException if bitLength &lt; 0 or if bits.length is longer than necessary
   *     for a vector of bitLength bits or if bits.length is too short for a vector of bitLength
   *     bits.
   */
  ...
  /**
   * Exchanges a selection of bits between two BitVectors, where the bits to exchange are indicated
   * by a bit mask, also represented by a BitVector.
   *
   * @param b1 The first BitVector.
   * @param b2 The second BitVector.
   * @param mask A bit mask indicating which bit positions to exchange. Specifically, if
   *     mask.getBit(i) is a 1, then b1 and b2 will trade the bits in position i between each other,
   *     and otherwise they won't.
   ...
   *
   * @return true If all of the bits are equal to 0, and false otherwise. Note that if the length of
   *     the BitVector is 0, this method returns true.
   */
   ...
   *
   * @return true If all of the bits are equal to 1, and false otherwise. Note that if the length of
   *     the BitVector is 0, this method returns true.
   */
   ...
   *
   * @return true If there is at least one 1-bit, and false otherwise. Note that if the length of
   *     the BitVector is 0, this method returns false.
   */
   ...
   *
   * @return true If there is at least one 0-bit, and false otherwise. Note that if the length of
   *     the BitVector is 0, this method returns false.
   */
   ...
   * @return The value of the bit at position index, as an int. The least significant bit of the
   *     returned int will hold the value of the bit. The other 31 bits will be 0.
   ...
   *
   * @param index The index into the bit vector, which must be 0 &le; index &lt; length().
   * @param bitValue The value to set for the bit at position index. The least significant bit of
   *     this int is used.
   * @throws IndexOutOfBoundsException if index is negative, or if index &ge; length()
   */
  ...
  /**
   * Flips the value of the bit at the specified index (e.g., if it is a 0, then change to a 1; if
   * it is a 1, change to a 0).
   *
   * @param index The index into the bit vector, which must be 0 &le; index &lt; length().
   * @throws IndexOutOfBoundsException if index is negative, or if index &ge; length()
   */
   ...
   * @return Up to 32 bits beginning at the bit at index 32*i. For example, if i is 0, then this
   *     method will return the 32 bits beginning at index 32*0=0, i.e., bits 0 through 31. Bit 0
   *     will be in the least significant position, and bit 31 will be in the most significant
   *     position. If i is 1, then this method returns the 32 bits beginning with the bit at index
   *     32 (i.e., bits 32 through 63). If there are less than 32 bits beginning at index 32*i, then
   *     the relevant bits will be in the least significant positions of the returned int.
   ...
   *
   * @param i The block of bits set by this method begins at the bit with index 32*i. For example,
   *     if i is 0, then this method will set the 32 bits beginning at index 32*0=0, i.e., bits 0
   *     through 31. Bit 0 will be in the least significant position, and bit 31 will be in the most
   *     significant position. If i is 1, then this method sets the 32 bits beginning with the bit
   *     at index 32 (i.e., bits 32 through 63).
   * @param block The block of bits to set. The least significant bit of block will be stored at
   *     index 32*i.
   * @throws IndexOutOfBoundsException if i is negative, or if 32*i &ge; length()
   */
  ...
  /**
   * Computes the bitwise AND of this BitVector and another BitVector. This BitVector is updated
   * with the result.
   *
  ...
  /**
   * Computes the bitwise OR of this BitVector and another BitVector. This BitVector is updated with
   * the result.
   *
   * @param other The other BitVector.
   * @throws IllegalArgumentException if the BitVectors are of different lengths.
  ...
  /**
   * Computes the bitwise XOR of this BitVector and another BitVector. This BitVector is updated
   * with the result.
   *
   * @param other The other BitVector.
   * @throws IllegalArgumentException if the BitVectors are of different lengths.
   ...
   * Specifically, the value of the bit in index i before a call to shiftLeft, will move to index (i
   * + numBits). The method does not change the length of the BitVector, so the numBits bits on the
   * left end of the BitVector before the call are lost.
   *
   ...
   * Specifically, the value of the bit in index i before a call to shiftLeft, will move to index (i
   * - numBits). The numBits bits on the right end of the BitVector before the call are lost.
   *
   ...
   *
   * @param k The number of bits for the BitIterator to return at a time. For example, if k is 3,
   *     then each call to {@link BitIterator#nextBitBlock()} will return an int containing the next
   *     3 bits from the BitVector in the least significant 3 places. The value of k must be 0 &lt;
   *     k &le; 32.
  ...
  /**
   * The BitIterator class enables iterating over the bits of a BitVector. In particular, it enables
   * iterating over fixed length blocks within the BitVector. BitIterators are created via the
   * {@link BitVector#bitIterator(int)} method or the {@link BitVector#bitIterator()} method.
   *
   * <p>An example usage of the BitIterator class is as follows. In this example, we are iterating
   * over the bits of a random BitVector of 128 bits, 4 bits at a time, using the {@link
   * #nextBitBlock()} method.
   *
   * <pre><code>
   ...
   * </code></pre>
   *
   * <p>The BitIterator class also includes a {@link #nextBit} method that gets the next single bit
   ...
   * </code></pre>
   *
   * <p>You can even use a combination of calls to the {@link #nextBitBlock()} method and the {@link
   * #nextBit} method. In this example, we alternate getting blocks of 4 bits with getting a single
   * bit.
   *
   * <pre><code>
   * BitVector b = new BitVector(128, true);
   ...
   * </code></pre>
   *
   * <p>There is also a {@link #nextBitBlock(int)} method, which enables overriding the default
   * block size for some calls. The class supports usages involving combinations of calls to {@link
   * #nextBitBlock()}, {@link #nextBitBlock(int)}, and {@link #nextBit()}. This next example
  ...
  public final class BitIterator {
     ...
     *
     * @return true if there are more bits, and false otherwise. If this method returns false, then
     *     calls to {@link nextBitBlock} will throw an exception.
     */
     ...
     *
     * @return the next block of bits, which will be in the least significant places of the returned
     *     int value.
     ...
     * Gets the next block of bits from the BitVector.
     *
     * @param k The block size, which overrides the default block size that was used when the
     *     BitIterator was initiated for this call only. For example, if k is 3, then this call will
     *     return an int containing the next 3 bits from the BitVector in the least significant 3
     *     places. The value of k must be 0 &lt; k &le; 32.
     * @return the next block of bits, which will be in the least significant places of the returned
     *     int value.
     * @throws IllegalArgumentException if k &le; 0 or if k &gt; 32
     * @throws IllegalStateException if there are no more blocks to get
     ...
     *
     * @param k The block size, which overrides the default block size that was used when the
     *     BitIterator was initiated for this call only. Unlike the {@link #nextBitBlock} methods,
     *     this method is not limited in block size, other than by the number of remaining bits.
     * @return the next block of bits, as an array of ints, with the first 32 bits of the block
     *     filled from least significant to most significant bit of a[0], where a is the array that
     *     is returned, and the next 32 bits filled similarly into a[1], etc.
    ...
    /**
     * Gets the next bit from the BitVector. Unlike the {@link #nextBitBlock()} method, this method
     * just gets 1 bit at a time, rather than a block of bits.
     *
     * @return the next bit, which will be in the least significant place of the returned int value.
