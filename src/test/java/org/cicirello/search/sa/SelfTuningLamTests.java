...
public class SelfTuningLamTests {
  ...
  public void testSplit() {
    ...
    m.accept(3, 2);
    assertEquals(
        0.44 * Math.pow(440, -1.0 / 35.0),
        m.getTargetRate(),
        EPSILON,
        "target rate start of phase 3");
  ...
  public void testTargetRate() {
    ...
    m.accept(3, 2);
    assertEquals(
        LAM_RATE_ONE_PERCENT_OF_RUN, m.getTargetRate(), EPSILON, "target rate after tuning");
    ...
    for (int i = 0; i < 10; i++) m.accept(3, 2);
    assertEquals(
        LAM_RATE_POINT_ONE_PERCENT_OF_RUN, m.getTargetRate(), EPSILON, "target rate after tuning");
