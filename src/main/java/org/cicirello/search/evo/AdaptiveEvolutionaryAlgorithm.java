 ...
 * rates and mutation rates that evolve during the search). It follows a generational model, where a
 * population of children are formed by applying genetic operators to members of the parent
 * population, and the children replace the parents in the next generation. It uses the typical
 * generational model using both crossover and mutation, such that each child may be the result of
 * crossover alone, mutation alone, a combination of both crossover and mutation, or a simple copy
 * of a parent.
 *
 * <p>Rather than specifying crossover and mutation rates, this adaptive evolutionary algorithm
 * evolves these during the search. Each member of the population consists of an encoding of a
 * candidate solution to the problem, along with a crossover rate C<sub>i</sub>, a mutation rate
 * M<sub>i</sub>, and a parameter &sigma;<sub>i</sub>. During a generation, parents are paired at
 * random. Consider that i and j are parents. One of these is chosen arbitrarily. For example,
 * consider that i was chosen. With probability C<sub>i</sub> the crossover operator is applied to
 * the parents, and otherwise it is not. Then, the mutation operator is applied to each member of
 * the population i with probability M<sub>i</sub>. Note that this class implements an evolutionary
 ...
 * operator is.
 *
 * <p>After applying the genetic operators, all of the C<sub>i</sub> and M<sub>i</sub> are
 * themselves mutated. Specifically, each is mutated with a Gaussian mutation with standard
 * deviation &sigma;<sub>i</sub>. The &sigma;<sub>i</sub> are then also mutated by a Gaussian
 * mutation with standard deviation of 0.01. The C<sub>i</sub> and M<sub>i</sub> are initialized
 * randomly at the start such that they are each in the interval [0.1, 1.0], and the Gaussian
 * mutation is implemented to ensure that they remain in that interval (e.g., reset to 0.1 if it is
 * ever too low, and to 1.0 if it is ever too high). The &sigma;<sub>i</sub> are initialized
 ...
 */
public class AdaptiveEvolutionaryAlgorithm<T extends Copyable<T>>
  ...
  public AdaptiveEvolutionaryAlgorithm(
    ...
    this(
        new EvolvableParametersPopulation.Double<T>(
  ...
  public AdaptiveEvolutionaryAlgorithm(
      ...
      ProgressTracker<T> tracker) {
    this(
        new EvolvableParametersPopulation.Integer<T>(
            n, initializer, f, selection, tracker, eliteCount, 2),
        f.getProblem(),
        mutation,
        crossover);
