...
/**
 * Generates random {@link SingleInteger} objects for use in generating random initial solutions for
 * simulated annealing and other metaheuristics, and for copying such objects. This initializer
 * supports both unbounded objects ({@link SingleInteger}) as well as bounded reals, where the
 * domain of values is bound in an interval. In the bounded case, the objects created by this class
 * enforce the bounds upon calls to {@link SingleInteger#set} such that the {@link
 * SingleInteger#set} method will set the value to the min if a value is passed less than min (and
 * similarly for max).
 *
...
public class IntegerValueInitializer implements Initializer<SingleInteger> {
   ...
   * [a, b). The {@link SingleInteger} objects returned by the {@link #createCandidateSolution}
   * method are otherwise unbounded (i.e., future mutations may alter the value such that it leaves
   * that interval). Use the other constructor if you need to enforce bounds.
   *
  ...
  /**
   * Construct a IntegerValueInitializer that generates random solutions uniformly in the interval
   * [a, b), subject to bounds [min, max]. If this constructor is used, then the {@link
   * #createCandidateSolution} method will return an object of a subclass of {@link SingleInteger},
   ...
   * attempt is made to set the value of this function input to a value less than the minimum, then
   * it is instead set to the minimum. If an attempt is made to set the value of this function input
   * to a value greater than the maximum, then it is instead set to the maximum.
   */
  private final class BoundedInteger extends SingleInteger {
    ...
    /**
     * Sets this function input to a specified value, subject to the lower and upper bounds for this
