...
/**
 * This is an implementation of a variation of the weighted critical ratio heuristic. The usual
 * definition of this heuristic is: h(j) = (w[j]/p[j])(1/(1+S(j)/p[j])), where w[j] is the weight of
 * job j, p[j] is its processing time, and S(j) is a calculation of the slack of job j where slack
 * S(j) is d[j] - T - p[j] - s[j]. The d[j] is the job's due date, T is the current time, and s[j]
 * is any setup time of the job (for problems with setup times).
 *
