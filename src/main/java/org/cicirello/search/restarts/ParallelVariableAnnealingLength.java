 ...
 * restart schedule.
 *
 * <p>The motivation underlying P-VAL is two-fold. First, a commonly encountered observation is that
 ...
 *
 * <p>The sequential version of the restart schedule, known as VAL, starts with a short run, and
 * increases run length exponentially across restarts. Specifically, define r<sub>i</sub> as the run
 * length for run i, with the following: r<sub>i</sub> = 1000 * 2<sup>i</sup>. For simulated
 * annealing, run length is number of evaluations (i.e., iterations of the simulated annealing main
 * loop). You can compute the sequence of run lengths incrementally with r<sub>0</sub> = 1000 and
 * r<sub>i</sub> = 2r<sub>i-1</sub>. The first few run lengths in the sequence are: 1000, 2000,
 * 4000, ....
 *
 * <p>P-VAL is a parallel version of VAL. Its essence is the same sequence of restarts, but those
 ...
 * same schedule. The run length, r<sub>i,t</sub>, of restart i of thread t is as follows:
 * r<sub>i,t</sub> = 1000 * 2<sup>(t mod 4) + i*min(4,N)</sup>, where i begins at 0, N is the number
 ...
 *
 * <p>Example 1 (N=3): Thread 0 follows the schedule of run lengths: 1000, 8000, 64000, .... Thread
 ...
 * Thread 1 follows the schedule of run lengths: 2000, 32000, 512000, .... Thread 2 follows the
 * schedule of run lengths: 4000, 64000, 1024000, .... Thread 3 follows the schedule of run lengths:
 * 8000, 128000, 2048000, .... Thread t, where t&ge;4, follows the same schedule of run lengths as
 * thread (t-4).
 *
 * <p>See the original publication, referenced below, for the theoretical rationale for beginning
 ...
 * parameter to specify the initial run length r<sub>0,0</sub> as something other than 1000.
 *
 ...
 *
 * <p>Since this restart schedule assumes multiple threads, and since each thread requires its own
...
public final class ParallelVariableAnnealingLength implements RestartSchedule {
   ...
   */
  public static List<ParallelVariableAnnealingLength> createRestartSchedules(
    ...
    ArrayList<ParallelVariableAnnealingLength> schedules =
        new ArrayList<ParallelVariableAnnealingLength>(numThreads);
