 ...
 * within the attraction basin of the local optima. Thus, the local optima is a trap for a local
 * search algorithm. The Trap function is related to the {@link TwoMax} problem, but in the TwoMax
 * problem, more of the search space is within the attraction basin of the global optima than within
 * that of the local optima.
 *
 * <p>The Trap problem is to maximize the following fitness function, f(x), where x is a vector of n
 * bits. Let z = floor((3/4)n). If CountOfOneBits(x) &le; z, then f(x) = (8n/z)(z-c). Otherwise,
 * f(x) = (10n/(n-z))(c-z).
 *
 * <p>The global optimal solution is when x is all ones, which has a maximal value of 10*n. This
 * search landscape also has a local optima when x is all zeros, which has a value of 8*n. Only bit
 * vectors with at least 3/4 of the bits equal to a one are within the attraction basin of the
 * global optima.
 *
 * <p>The {@link #value value} method implements the original maximization version of the Trap
 ...
 *
 * <p>The Trap problem was introduced by David Ackley in the following paper:<br>
...
public final class Trap implements OptimizationProblem<BitVector> {
  ...
  public double cost(BitVector candidate) {
    ...
    if (c == z) {
      ...
      return 10
          * candidate.length()
          * (((double) (candidate.length() - c)) / (candidate.length() - z));
    }
  }
