...
/**
 * <p>This class implements a constructive heuristic, known as LINET, for scheduling problems
 * involving minimizing the sum of weighted earliness plus weighted tardiness.  LINET
 * is an acronym for Linear Early Tardy.</p>
 * 
 * <p>To define the LINET heuristic, first let he[j] be the weighted longest processing time heuristic
 * of job j, defined as he[j] = -we[j] / p[j], where we[j] is the earliness weight
 * for job j, and p[j] is the processing time of job j.  Next, let ht[j] be the weighted shortest
 ...
 *
 * <p>Now we can define the LINET heuristic, h[j] for job j, as follows.  If s[j] &le; 0, 
 * h[j] = ht[j]. Otherwise, if s[j] &ge; k*p&#772;, h[j] = he[j].  Otherwise,
 * h[j] = ht[j] - s[j](ht[j]-he[j])/(k*p&#772;).  That is, for jobs with negative slack, the
 * LINET heuristic is equivalent to weighted shortest processing time.  For jobs with slack
 * greater than some multiple of the average processing time, LINET is equivalent to 
 * weighted longest processing time.  There is then a linear transition between the two
 * extremes.</p>
 *
 * <p>We make one additional adjustment to the heuristic as it was originally described.
 * Since this library's implementations of stochastic sampling algorithms assumes that constructive
 * heuristics always produce positive values, we must adjust the values produced by the LINET heuristic.
