 ...
 * The earliness of a job j is: E[j] = max(0, -L[j]).
 * The weighted earliness plus weighted tardiness problem involves
 * a pair of weights for each job, a tardiness weight wt[j] and an
 * earliness weight we[j].
 * The cost function
 * is the weighted sum over the jobs of: wt[j]T[j] + we[j]E[j],
