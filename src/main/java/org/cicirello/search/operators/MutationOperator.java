 ...
 *
 * <p>If your mutation operator is one in which the inverse operation (i.e., the operation that
 * reverts an object to its previous state from prior to the mutation) can be implemented without
 * substantially affecting the runtime of the mutation itself, then consider implementing the {@link
 * UndoableMutationOperator} interface instead, a subinterface of MutationOperator that adds an undo
 * method.
 *
 * <p>On the other hand, if implementing {@link UndoableMutationOperator#undo} would require
 * significant added cost in either time or memory to the {@link #mutate} method, then consider
 * implementing two versions of your mutation operator, one that implements MutationOperator and a
 * second that implements {@link UndoableMutationOperator}. In this way, you can use the first
...
public interface MutationOperator<T> extends Splittable<MutationOperator<T>> {
  ...
  /**
   * Mutates a candidate solution to a problem, by randomly modifying its state. The mutant that is
   * produced is in the local neighborhood of the original candidate solution.
   *
   * @param c The candidate solution subject to the mutation. This method changes the state of c.
   */
  void mutate(T c);
}
