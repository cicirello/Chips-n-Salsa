...
/**
 * The nested classes are for populations with double-valued and int-valued fitnesses
 * for use by EAs with parameters that evolve during the search.
 *
...
abstract class EvolvableParametersPopulation {
	 ...
	 * and such that parameters evolve during the search. 
	 *
	...
	static final class Double<T extends Copyable<T>> extends AbstractPopulation<T> implements PopulationFitnessVector.Double {
		 ...
		 */
		private Double(EvolvableParametersPopulation.Double<T> other) {
			super(other);
			...
			pop = new ArrayList<PopulationMember.DoubleFitness<EncodingWithParameters<T>>>(MU);
			nextPop = new ArrayList<PopulationMember.DoubleFitness<EncodingWithParameters<T>>>(LAMBDA);
			elite = other.elite != null ? new EliteSet.DoubleFitness<EncodingWithParameters<T>>(MU - LAMBDA) : null;
			selected = new int[LAMBDA];
			updated = new boolean[LAMBDA];
			bestFitness = java.lang.Double.NEGATIVE_INFINITY;
		}
		...
		@Override
		public EvolvableParametersPopulation.Double<T> split() {
			return new EvolvableParametersPopulation.Double<T>(this);
		}
		...
		public void updateFitness(int i) {
			...
			if (fit > bestFitness) {
				bestFitness = fit;
				setMostFit(f.getProblem().getSolutionCostPair(nextPop.get(i).getCandidate().getCandidate().copy())); 
			}
		}
	...
	static final class Integer<T extends Copyable<T>> extends AbstractPopulation<T> implements PopulationFitnessVector.Integer {
		 ...
		 * private constructor for use by split.
		 */
		private Integer(EvolvableParametersPopulation.Integer<T> other) {
			super(other);
			...
			pop = new ArrayList<PopulationMember.IntegerFitness<EncodingWithParameters<T>>>(MU);
			nextPop = new ArrayList<PopulationMember.IntegerFitness<EncodingWithParameters<T>>>(LAMBDA);
			elite = other.elite != null ? new EliteSet.IntegerFitness<EncodingWithParameters<T>>(MU - LAMBDA) : null;
			selected = new int[LAMBDA];
			updated = new boolean[LAMBDA];
			bestFitness = java.lang.Integer.MIN_VALUE;
		}
		...
		@Override
		public EvolvableParametersPopulation.Integer<T> split() {
			return new EvolvableParametersPopulation.Integer<T>(this);
		}
