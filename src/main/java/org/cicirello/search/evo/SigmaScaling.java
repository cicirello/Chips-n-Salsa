...
/**
 * <p>Implements sigma scaling by wrapping your chosen selection operator. The SigmaScaling
 * instance applies sigma scaling to the fitnesses of the population, passing the scaled
 * fitnesses to the true selection operator.</p>
 *
 * <p>The fitness, f<sub>i</sub>, of population member i is transformed as follows:
 * f'<sub>i</sub> = f<sub>i</sub> - ( f&#773; - c * &sigma; ), where c is a parameter
 * usually between 1 and 3, inclusive. &sigma; is the standard deviation of the fitnesses of
 * the population, and f&#773; is average fitness. For any members of the population for which
 * this transformation produces a fitness less than 0.001, it is reset to 0.001.</p>
 *
...
public final class SigmaScaling implements SelectionOperator {
	...
	/**
	 * The minimum scaled fitness, such that any fitnesses that lead to scaled fitness less than
	 * this value, are reset to this value.
	 */
	public static final double MIN_SCALED_FITNESS = 0.001;
	
	/**
	 * Constructs a new SigmaScaling object, to transform fitness values via sigma scaling
	 * prior to selection by a wrapped selection operator. Uses a default c = 2.0.
	 *
	...
	/**
	 * Constructs a new SigmaScaling object, to transform fitness values via sigma scaling
	 * prior to selection by a wrapped selection operator.
	 *
	 * @param selection The selection operator.
