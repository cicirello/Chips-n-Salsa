...
/**
 * <p>This class implements the classic 3-Opt neighborhood as a mutation operator 
 * for permutations. The 3-Opt neighborhood includes all two-changes and all three-changes.
 * These originated specifically for the TSP. A two-change  
 * for the TSP removes two edges from a tour of the cities of a TSP and replaces
 * them with two different edges such that the result is a valid tour of the cities.
 * Likewise, a three-change removes three edges from the tour of the cities of a TSP
 * and replaces them with three different edges.
 ...
 * permutation represents a cyclic sequence of undirected edges, and specifically 
 ...
 * neighbors, and 4*(n*(n-1)*(n-2)/6 - n*(n-4) - n) + n*(n-4) possible 
 * three-change neighbors. Each of the possible three-changes is approximately equally
 * likely as every other three-change. Each of the possible two-changes is
 * approximately equally likely as every other two-change. The current implementation does
 * not guarantee that each of the possible two-changes are equally likely as each of the
 * possible three-changes. Currently, each two-change is slightly more likely than 
 * each three-change. Although because the number of possible three-changes grows cubicly
 * vs quadratic growth in number of possible two-changes, the probability of a three-change
 * increases rapidly as permutation length increases.</p>
 *
 * <p>For permutations of length equal to 4, the ThreeOptMutation
 * will only perform two-changes because no valid three-changes exist for that length.
 * For permutations of length n &lt; 4, the ThreeOptMutation operator
 * makes no changes, as there are no two-change or three-change neighbors 
 * of permutations of that size.</p> 
 *
