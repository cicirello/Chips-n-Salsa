 ...
 * class that implements the {@link Metaheuristic} interface, and requires specification of either a
 * {@link RestartSchedule} for the purpose of specifying run lengths for the restarts, or a run
...
public class Multistarter<T extends Copyable<T>> implements Metaheuristic<T> {
  ...
  /**
   * Gets the total run length of all restarts of the underlying metaheuristic combined. This may
   * differ from what may be expected based on run lengths passed to the optimize and reoptimize
   * methods of the underlying metaheuristic. For example, the optimize method terminates if it
   * finds the theoretical best solution, and also immediately returns if a prior call found the
   * theoretical best. In such cases, the total run length may be less than the requested run
   * length.
   *
   ...
   * begins at a new randomly generate initial state.
   *
