 ...
 * you won't instantiate this class directly (it doesn't have a public constructor),
 * its methods can be used to compute the cost of solutions, or to map a Permutation
 * to the details of the solution that it represents (i.e., which items are in which bins).
 * This class also provides methods for getting the sizes of items, and other information
 * about the instance. To generate instances of the Bin Packing problem, use the
 * constructors of the nested subclasses.
 * 
...
public class BinPacking implements IntegerCostOptimizationProblem<Permutation> {
	 ...
	 * @throws IndexOutOfBoundsException if i is either negative or greater than or equal to numItems()
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * <p>Computes the cost of the solution formed
	 * by applying a first-fit heuristic using the item
	 * ordering implied by the Permutation candidate. The optimal
	 * solution will have index of items for a bin grouped together
	...
	/**
	 * Determines the bin packing solution that corresponds to a given permutation,
	 * such that the solution is formed by applying a first-fit heuristic using the item
	 * ordering implied by the Permutation p.
	...
	public static final class UniformRandom extends BinPacking {
		...
		/**
		 * Generates a random instance of the Bin Packing problem, with a default
		 * bin capacity of 150, and with item sizes generated uniformly at random
		 * from the interval [20, 100]. These are the defaults of the uniform
		 ...
		 *
		 * @param numItems The number of items.
		 * @param seed The seed for the random number generator to enable reproducible problem instances.
		 *
		 * @throws NegativeArraySizeException if numItems is negative.
		 */
		...
		/**
		 * Generates a random instance of the Bin Packing problem, with item
		 * sizes generated uniformly at random from a specified interval.
		 *
		 * @param numItems The number of items.
	...
	/**
	 * <p>Generates instances of the Bin Packing problem where the optimal solution
	 * is comprised of all full triplet bins (each bin in optimal solution has exactly
	 * 3 items that fills the bin to capacity). The cost of the optimal solution to
	 * each such instance is one third the number of items.</p>
	 *
	 * <p>The bin capacity for the instances generated by this method is 1000. When generating
	 * the instance, each triplet is generated as follows. The size of one item is uniform
	 * at random from the interval [380, 490]. The second item's size is generated uniformly
	 * at random from [250, s/2), where s is 1000 - size of the first item. The third item for
	 * the triplet has size 1000 - s - t, where s and t are the sizes of the first two items.</p>
	 *
	 * <p>The triplets, as defined above, are as described in: "A Hybrid Grouping 
	 * Genetic Algorithm for Bin Packing" by Emanuel Falkenauer.</p>
	 *
	 * @author <a href=https://www.cicirello.org/ target=_top>Vincent A. Cicirello</a>, 
	 * <a href=https://www.cicirello.org/ target=_top>https://www.cicirello.org/</a>
	 */
	public static final class Triplet extends BinPacking {
		 ...
		 *
		 * @param numItemTriplets The number of triplets. The number of items in the instance
		 *    will equal 3 * numItemTriplets.
		 *
