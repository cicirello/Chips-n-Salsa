...
public class PermutationBitVectorTests {
  ...
  public void testConversionBaseClass() {
    ...
    int[] expectedBitLengths = {
      0, 1, 2 * 2, 2 * 3, 3 * 4, 3 * 5, 3 * 6, 3 * 7, 4 * 8, 4 * 9, 4 * 10, 4 * 11, 4 * 12, 4 * 13,
      4 * 14, 4 * 15
    };
    int[][] modCases = {
      {0},
      ...
      {0x789abcde, 0x23456},
      {0x89abcdef, 0x234567},
      {0x9abcdef0, 0x2345678}
    };
    for (int i = 0; i < expected.length; i++) {
      PermutationToBitVectorProblem converter =
          new PermutationToBitVectorProblem(expected[i].length());
      ...
      if (i < modCases.length && i > 0) {
        assertEquals(
            expected[i],
            converter.toPermutation(new BitVector(expectedBitLengths[i], modCases[i])));
      }
      ...
      if (i < modCases.length && i > 0) {
        assertEquals(
            expected[i], split.toPermutation(new BitVector(expectedBitLengths[i], modCases[i])));
      }
      ...
      if (i < reversedCases.length && i > 0) {
        assertEquals(
            expected[i],
            converter.toPermutation(new BitVector(expectedBitLengths[i], reversedCases[i])));
        assertEquals(
            expected[i],
  ...
  public void testIntegerCosts() {
    ...
    for (int i = 0; i < expected.length; i++) {
      PermutationInAHaystack problem =
          new PermutationInAHaystack(new ExactMatchDistance(), expected[i].copy());
      PermutationToBitVectorProblem.IntegerCost converter =
          new PermutationToBitVectorProblem.IntegerCost(problem, expected[i].length());
      assertEquals(expectedBitLengths[i], converter.supportedBitVectorLength());
      assertEquals(expectedBitLengths[i], converter.createCandidateSolution().length());
      assertEquals(expected[i], converter.toPermutation(new BitVector(expectedBitLengths[i])));
      if (i < modCases.length && i > 0) {
        ...
        assertEquals(
            0.0, converter.costAsDouble(new BitVector(expectedBitLengths[i], modCases[i])), 1E-10);
      ...
      SolutionCostPair<BitVector> pair =
          converter.getSolutionCostPair(new BitVector(expectedBitLengths[i]));
      ...
      if (i < reversedCases.length && i > 0) {
        ...
        assertEquals(
            ...
            split.toPermutation(new BitVector(expectedBitLengths[i], reversedCases[i])));
        assertEquals(
            expected[i].length() % 2 == 0 ? expected[i].length() : expected[i].length() - 1,
            converter.cost(new BitVector(expectedBitLengths[i], reversedCases[i])));
        assertEquals(
            expected[i].length() % 2 == 0 ? expected[i].length() : expected[i].length() - 1,
            converter.value(new BitVector(expectedBitLengths[i], reversedCases[i])));
        assertEquals(
            expected[i].length() % 2 == 0 ? expected[i].length() : expected[i].length() - 1,
            converter.costAsDouble(new BitVector(expectedBitLengths[i], reversedCases[i])),
            1E-10);
        pair =
            converter.getSolutionCostPair(new BitVector(expectedBitLengths[i], reversedCases[i]));
        assertEquals(
            expected[i].length() % 2 == 0 ? expected[i].length() : expected[i].length() - 1,
            pair.getCost());
    ...
    IllegalArgumentException thrown =
        assertThrows(
            ...
            () ->
                new PermutationToBitVectorProblem.IntegerCost(
                    new PermutationInAHaystack(new ExactMatchDistance(), 10), 0));
  }
  ...
  public void testDoubleCosts() {
    ...
    for (int i = 0; i < expected.length; i++) {
      ...
      PermutationToBitVectorProblem.DoubleCost converter =
          new PermutationToBitVectorProblem.DoubleCost(problem, expected[i].length());
      assertEquals(expectedBitLengths[i], converter.supportedBitVectorLength());
      assertEquals(expectedBitLengths[i], converter.createCandidateSolution().length());
      assertEquals(expected[i], converter.toPermutation(new BitVector(expectedBitLengths[i])));
      if (i < modCases.length && i > 0) {
        ...
        assertEquals(
            0.0, converter.costAsDouble(new BitVector(expectedBitLengths[i], modCases[i])), 1E-10);
        assertEquals(
            0.0, converter.value(new BitVector(expectedBitLengths[i], modCases[i])), 1E-10);
      }
    ...
    IllegalArgumentException thrown =
        assertThrows(
            IllegalArgumentException.class,
            () ->
                new PermutationToBitVectorProblem.DoubleCost(
                    new TestProblem(new Permutation(10)), 0));
  }
