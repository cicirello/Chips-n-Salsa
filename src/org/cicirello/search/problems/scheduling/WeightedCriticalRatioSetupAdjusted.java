 ...
 * sequence-dependent setups.  The usual definition of this heuristic is:
 ...
 * current time, and s[i][j] is setup time of the job if it follows job i
 * (for problems with setup times).</p>  
 *
 ...
 *
 * <p>Finally, to adjust for setup times, we replace p[j] wherever it
 * appears with p[j]+s[i][j].  This leads to:
 * h(j) = (w[j]/(p[j]+s[i][j]))(1/(1+max(0,S(j))/(p[j]+s[i][j]))).</p>
 *
