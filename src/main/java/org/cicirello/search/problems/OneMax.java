 ...
 *
 * <p>It was originally posed as a maximization problem because it was originally defined as a
 * fitness function for a genetic algorithm. The {@link #value value} method simply counts the
 * number of bits in the BitVector equal to 1, which is to be maximized. Thus, as a cost function,
 * the {@link #cost cost} method counts the number of bits not equal to 1, where the minimum cost is
 * thus 0, corresponding to the case of maximal number of 1-bits.
 *
 * <p>The OneMax problem was introduced by Ackley (1985). His original definition of the problem was
 ...
 * can use the {@link IntegerCostFunctionScaler} class for this purpose. You can do so by defining
 * your optimization problem with something like: IntegerCostFunctionScaler&lt;BitVector&gt; problem
 * = new IntegerCostFunctionScaler&lt;BitVector&gt;(new OneMax()); Additionally, the {@link
 * OneMaxAckley} class specifically implements Ackley's version with the costs scaled by a factor of
 * 10.
 *
 * <p>Although commonly used by others without reference, the OneMax problem was introduced by David
 * Ackley in the following paper:<br>
