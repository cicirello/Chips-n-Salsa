...
/**
 * Implements the scheduling cost function known as weighted tardiness. The lateness L[j] of job j
 * is: L[j] = C[j] - d[j], where C[j] is the time it is completed by the machine, and d[j] is its
 * due date. The tardiness of job j is: T[j] = max(0, L[j]). That is, although lateness can be
 * negative, tardiness is never negative (i.e., no reward for a job completing early). The weighted
 * tardiness cost function is the weighted sum over the jobs of: w[j]T[j], where w[j] is job j's
 * weight.
 *
