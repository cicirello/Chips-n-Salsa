 ...
 * {@link BlockMoveMutation} mutation operator on permutations.  A window-limited
 * mutation operator on permutations is a mutation operator such that there is a window 
 * constraint, w, on the random indexes used by the mutation.  For a mutation operator like
 * {@link BlockMoveMutation} that uses three random indexes (i, j, k), 
 * two to define the block and the third for the reinsertion point,
 * these indexes are constrained such that |i-j| &le; w, |i-k| &le; w, |j-k| &le; w.
 * All index triples that satisfy the window constraint are equally likely.</p>
...
public final class WindowLimitedBlockMoveMutation extends BlockMoveMutation {
	...
	/**
	 * Constructs a WindowLimitedBlockMoveMutation mutation operator with
