...
/**
 * <p>This class implements Boltzmann selection using Stochastic Universal Sampling (SUS). 
 * Boltzmann selection is similar to
 * a fitness proportional selection, except instead of a population member being 
 * weighted by its fitness f in the randomized selection process, Boltzmann selection
 * weights it by e<sup>f/T</sup>, where f is the fitness of the individual, and T is
 * a temperature parameter, much like that of simulated annealing. T typically decreases
 * over the run of the evolutionary algorithm.</p> 
 *
 * <p>This implementation supports a constant temperature T, as well as 
 * two cooling schedules: linear cooling and
 * exponential cooling. In both cases, at the start of the evolutionary
 * algorithm, the temperature T is initialized to a t0. In linear cooling, 
 * at the end of each generation, T is updated according to: T = T - r.
 * In exponential cooling, at the end of each generation, T is updated according
 * to: T = r * T. In both cases, if T ever falls below some tMin, it is reset to
 * tMin.</p>
 *
 * <p>Unlike many other fitness proportional related selection operators, Boltzmann
 * selection, including this SUS version, is applicable even if fitness values can be negative.</p>
 *
 * <p>SUS and this Boltzmann SUS are similar to fitness proportional selection and a variation of
 * fitness proportional selection biasing selection by the Boltzmann distribution. However, whereas fitness 
 ...
 *
 * <p>For the basic version of Boltzmann selection, see the {@link BoltzmannSelection} class. And for the 
 * standard version of SUS, see the {@link StochasticUniversalSampling} class.</p>
 *
...
public final class BoltzmannStochasticUniversalSampling extends BiasedStochasticUniversalSampling {
	...
	/**
	 * Construct a Boltzmann selection operator, with either a linear cooling schedule
	 * or an exponential cooling schedule.
	 *
	 ...
	 * @param r The update value, which must be positive for linear cooling, and must be in (0.0, 1.0) for
	 *    exponential cooling.
	 * @param linearCooling If true, uses a linear cooling schedule, and if false it uses an exponential cooling schedule.
	 *
	 * @throws IllegalArgumentException if tMin is not positive or if t0 is less than tMin or if linear cooling with non-positive r
	 *    or if exponential cooling with r not in (0,0, 1.0).
	 */
	public BoltzmannStochasticUniversalSampling(double t0, double tMin, double r, boolean linearCooling) {
		this(linearCooling ? 
		...
		if (t0 < tMin) throw new IllegalArgumentException("Minimum temperature must be no greater than initial temperature.");
		if (r <= 0.0) throw new IllegalArgumentException("r must be positive");
		if (!linearCooling && r >= 1.0) throw new IllegalArgumentException("For exponential cooling, r must be positive.");
	}
