 ...
 * fitness.
 */
final class EliteSetIntegerFitness<T extends Copyable<T>>
  ...
  EliteSetIntegerFitness(int numElite) {
   ...
   * package-private for use by classes in evo package for maintaining a set of elite population members.
   */
  EliteSetIntegerFitness(ArrayList<PopulationMember.IntegerFitness<T>> initialPop, int numElite) {
    this(numElite);
    offerAll(initialPop);
  }
