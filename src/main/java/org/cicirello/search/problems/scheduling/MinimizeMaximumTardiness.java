 ...
 * The lateness L[j] of job j is: L[j] = C[j] - d[j], where C[j] is the time it is completed by the
 * machine, and d[j] is its due date. The tardiness T[j] = max(0, L[j]). So although lateness can be
 * negative, tardiness is never negative. The maximum tardiness cost function is equal to:
 * max<sub>j</sub> { T[j] }.
 *
