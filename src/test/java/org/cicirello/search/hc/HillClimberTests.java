...
public class HillClimberTests {
	...
	public void testConstructorsSteepestDescent() {
		...
		SteepestDescentHillClimber<TestObject> hc = new SteepestDescentHillClimber<TestObject>(problem, mutation, init, tracker);
		...
		NullPointerException thrown = assertThrows( 
			...
			() -> new SteepestDescentHillClimber<TestObject>((OptimizationProblem<TestObject>)null, mutation, init, tracker)
		);
		thrown = assertThrows( 
			NullPointerException.class,
		...
		thrown = assertThrows( 
			NullPointerException.class,
			() -> new SteepestDescentHillClimber<TestObject>((IntegerCostOptimizationProblem<TestObject>)null, mutation, init, tracker)
		);
		thrown = assertThrows( 
			NullPointerException.class,
	...
	public void testIntSteepest() {
		TestObject.setB(0);
		SteepestDescentHillClimber<TestObject> hc = new SteepestDescentHillClimber<TestObject>(new TestOptInt(), new TestMutator(), new TestObject(1000));
	...
	public void testSteepestSplitDouble() {
		for (int r = 1; r <= 5; r++) {
			for (int k = 0; k < r-1; k++) {
				TestObject.setB(r);
				SteepestDescentHillClimber<TestObject> hc = new SteepestDescentHillClimber<TestObject>(new TestOpt(), new TestMutator(), new TestObject(1000));
				assertEquals(0, hc.getTotalRunLength());
				SolutionCostPair<TestObject> s = null;
				for (int i = 0; i < r; i++) {
					if (k==i) {
	...
	public void testIntFirst() {
		TestObject.setB(0);
		FirstDescentHillClimber<TestObject> hc = new FirstDescentHillClimber<TestObject>(new TestOptInt(), new TestMutator(), new TestObject(1000));
		assertEquals(0, hc.getTotalRunLength());
		SolutionCostPair<TestObject> s = hc.optimize();
	...
	public void testDoubleFirst() {
		TestObject.setB(0);
		FirstDescentHillClimber<TestObject> hc = new FirstDescentHillClimber<TestObject>(new TestOpt(), new TestMutator(), new TestObject(1000));
		assertEquals(0, hc.getTotalRunLength());
		SolutionCostPair<TestObject> s = hc.optimize();
		assertEquals(26, hc.getTotalRunLength());
		assertEquals(2.0*TestObject.OPT, s.getCostDouble(), EPSILON);
	...
	private static class TestOpt implements OptimizationProblem<TestObject> {
		...
		@Override public double cost(TestObject c) {
			return 2.0*c.a;
		}
		
		@Override public double value(TestObject c) {
			return cost(c);
		}
		
		@Override public boolean isMinCost(double cost) {
			return cost == minCost();
		}
		
		@Override public double minCost() {
	...
	private static class TestOptInt implements IntegerCostOptimizationProblem<TestObject> {
		...
		@Override public int value(TestObject c) {
			return cost(c);
		}
		
		@Override public boolean isMinCost(int cost) {
			return cost == minCost();
		}
		
		@Override public int minCost() {
	...
	private static class TestObject implements Copyable<TestObject>, Initializer<TestObject> {
		...
		final static int OPT = 85;
		static int B;
		public static void setB(int b) {B = b;}
		
		public TestObject(int a) {this.a = a; b = 0;}
		
		public TestObject(int a, int b) {this.a = a; this.b = b;}
