 ...
 * - p[j]) / (k<sub>1</sub> p&#772;) ) exp( -s[i][j] / (k<sub>2</sub> s&#772;)), where w[j] is the
 * weight of job j, p[j] is its processing time, d[j] is the job's due date, T is the current time,
 * and s[i][j] is any setup time of the job if it follows job i. The k<sub>1</sub> and k<sub>2</sub>
