...
/**
 * This is an implementation of the Apparent Tardiness Cost (ATC) heuristic. ATC is defined as: h(j)
 * = (w[j]/p[j]) exp( -max(0,S(j)) / (k p&#772;) ), where w[j] is the weight of job j, p[j] is its
 * processing time, and S(j) is a calculation of the slack of job j where slack S(j) is d[j] - T -
 * p[j] - s[j]. The d[j] is the job's due date, T is the current time, and s[j] is any setup time of
 * the job (for problems with setup times). The k is a parameter that can be tuned based on problem
 * instance characteristics, and p&#772; is the average processing time of remaining unscheduled
 * jobs.
 *
