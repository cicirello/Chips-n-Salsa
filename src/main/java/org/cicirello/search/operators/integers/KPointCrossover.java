...
/**
 * Implementation of K-point crossover, but for IntegerVectors. K-point crossover is a
 * generalization of two-point crossover. In a K-point crossover, K random cross points are chosen
 * uniformly along the length of the parents. Both parents are cut at the K cross points, breaking
 * the parents into multiple segments. The elements within every other segment are then swapped
 * between the two parents to form the two children. K-point crossover originated for crossover of
 * bit vectors within a genetic algorithm, but has been adapted here for vectors of integers.
 *
...
public final class KPointCrossover<T extends IntegerVector> implements CrossoverOperator<T> {
  ...
  public void cross(IntegerVector c1, IntegerVector c2) {
    ...
    for (; i < indexes.length; i += 2) {
      IntegerVector.exchange(c1, c2, indexes[i - 1], indexes[i] - 1);
    }
    i--;
    if (i < indexes.length) {
      IntegerVector.exchange(c1, c2, indexes[i], c1.length() - 1);
    }
  }
