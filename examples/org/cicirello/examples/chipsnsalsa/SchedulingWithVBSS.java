...
public class SchedulingWithVBSS {
	...
	public static void main(String[] args) {
		...
		// (the default if left out is linear bias, but we use quadratic bias in this
		// example).  You are not limited to polynomial bias functions, as there is
		// a constructor that enables passing a custom bias function.
		ValueBiasedStochasticSampling vbss = new ValueBiasedStochasticSampling(heuristic, 2);
		...
		FirstDescentHillClimber<Permutation> hybrid = new FirstDescentHillClimber<Permutation>(
			...
			new InitializeBySimpleMetaheuristic<Permutation>(new ValueBiasedStochasticSampling(heuristic, 2))
		);
		...
		for (int totalSamples = 1, executedSamples = 0; totalSamples <= 10000; totalSamples *= 10) {
