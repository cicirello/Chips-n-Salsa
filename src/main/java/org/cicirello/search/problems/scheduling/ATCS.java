...
/**
 * This is an implementation of the ATCS (Apparent Tardiness Cost with Setups) heuristic. ATCS is
 * defined as: h(j) = (w[j]/p[j]) exp( -max(0,d[j] - T - p[j]) / (k<sub>1</sub> p&#772;) ) exp(
 * -s[i][j] / (k<sub>2</sub> s&#772;)), where w[j] is the weight of job j, p[j] is its processing
 * time, d[j] is the job's due date, T is the current time, and s[i][j] is any setup time of the job
 * if it follows job i. The k<sub>1</sub> and k<sub>2</sub> are parameters that can be tuned based
 * on problem instance characteristics, p&#772; is the average processing time of all jobs, and
 * s&#772; is the average setup time of all jobs. The p&#772; and s&#772; are computed once at the
 * start. For a version of the heuristic that dynamically updates these, see the {@link DynamicATCS}
 * class.
 *
...
public final class ATCS extends WeightedShortestProcessingTime {
  ...
  public ATCS(SingleMachineSchedulingProblem problem, double k1, double k2) {
    ...
    if (totalSetupTimes == 0) {
      ...
      sAve = ((double) totalSetupTimes) / (n * n);
  ...
  public double h(Partial<Permutation> p, int element, IncrementalEvaluation<Permutation> incEval) {
    ...
    if (ADJUST_FOR_SETUPS) {
      return adjustHForSetups(adjustHForDuedates(value, element, incEval), p, element);
    }
  ...
  private double adjustHForDuedates(
    ...
    double num =
        data.getDueDate(element)
            ...
            - ((IncrementalTimeCalculator) incEval).currentTime();
    if (num > 0) {
      value *= Math.exp(-num / (k1 * pAve));
      if (value <= MIN_H) {
        return MIN_H;
      }
    }
  ...
  private double adjustHForSetups(double value, Partial<Permutation> p, int element) {
    double num =
        p.size() == 0 ? data.getSetupTime(element) : data.getSetupTime(p.getLast(), element);
    if (num > 0) {
      value *= Math.exp(-num / (k2 * sAve));
      if (value <= MIN_H) {
        return MIN_H;
      }
    }
