 ...
 * objects.
 *
 * <p>When calling the {@link #optimize optimize} method, the runLength parameter corresponds to the
 * number of restarts for each of the Multistarter instances, where those restarts will have run
 * lengths determined by the restart schedule specified upon construction. If the {@link #optimize
 * optimize} method is called multiple times, the restart schedules of the parallel metaheuristics
...
public final class ParallelMultistarter<T extends Copyable<T>> extends ParallelMetaheuristic<T> {
   ...
   */
  public ParallelMultistarter(
      Metaheuristic<T> search, Collection<? extends RestartSchedule> schedules) {
    super(toMultistarters(search, schedules), false);
  }
  ...
  public ParallelMultistarter(
    ...
    super(toMultistarters(searches, schedules), false);
  }
   ...
   */
  private static <T2 extends Copyable<T2>> ArrayList<Metaheuristic<T2>> toMultistarters(
