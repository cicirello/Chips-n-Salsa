...
/**
 * <p>This class implements linear rank selection using Stochastic Universal Sampling (SUS). 
 * Linear rank selection begins
 ...
 * proportional to: 2 - c + 2(r - 1)(c - 1)/(N - 1). The c is a real-valued parameter
 * that must be in the interval [1, 2]. When c is equal to 1, all population
 * members are equally likely chosen. When c is equal to 2, the expected number of
 * times the most fit population member will be chosen is 2, the least fit member
 * won't be selected at all, and the expected number of times the other population
 * members will be chosen in a generation will vary between 0 and 2 based upon rank.
 * To avoid a probability of 0 of choosing the least fit population member, then
 * ensure that c is less than 2. To ensure that the selection operator doesn't
 * degenerate into a uniform random selection, then set c greater than 1. The value
 * of c can be interpreted as the expected number of times the most fit population
 * member will be selected in a generation.</p>
 *
 * <p>Linear rank selection was introduced by Baker (1985). According to "An Introduction
 * to Genetic Algorithms" (Melanie Mitchell, 1998), Baker recommended c = 1.1.</p>
 *
...
public final class LinearRankStochasticUniversalSampling extends StochasticUniversalSampling {
	 ...
	 * @param c The expected number of times the most fit population member should be
	 * selected during one generation, which must be in the interval [1.0, 2.0].
	 *
	...
	final double[] computeWeightRunningSum(PopulationFitnessVector.Integer fitnesses) {
		return computeWeightRunningSumRanks(
			...
			r -> 2 - c + 2 * r * (c - 1) / (fitnesses.size()-1.0)
		);
	}
