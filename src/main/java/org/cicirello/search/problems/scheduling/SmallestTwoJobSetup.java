...
/**
 * This heuristic is smallest two-job setup. We define it as: h(j) = 1 / (1 + s[i][j] +
 * min<sub>k</sub>(s[j][k])), where s[i][j] is the setup time of job j if it follows job i on the
 * machine, job i is the most recently schedule job, and min<sub>k</sub>(s[j][k]) is the minimum
 * setup of potential successors of job j.
 *
