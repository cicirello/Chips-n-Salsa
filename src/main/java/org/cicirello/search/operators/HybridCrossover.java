 ...
 * evolutionary algorithm, such that each time the {@link #cross} method is called,
 * a randomly chosen crossover operator is applied to the candidate
 * solution.  The random choice of crossover operator is approximately uniform
 * from among the available crossover operators.
 *
...
public final class HybridCrossover<T> implements CrossoverOperator<T> {
	...
	public HybridCrossover(Collection<? extends CrossoverOperator<T>> crossoverOps) {
		if (crossoverOps.size() == 0) throw new IllegalArgumentException("Must pass at least 1 CrossoverOperator.");
