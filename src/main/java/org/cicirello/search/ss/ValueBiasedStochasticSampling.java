 ...
 *
 * <p>Although the VBSS algorithm itself is not restricted to permutation problems, the examples
 ...
 *
 * <p>Now, VBSS also uses a bias function b. We will compute b(h(p, e)) for each element e under
 * consideration for addition to partial permutation p. A common form of the bias function is b(h(p,
 * e)) = h(p, e)<sup>a</sup> for some exponent a. The greater your confidence is in the decision
 ...
 *
 * <p>To use this implementation of VBSS, you will need to implement a constructive heuristic for
 ...
 * heuristic is O(f(L)), the runtime to construct one permutation using VBSS is O(L<sup>2</sup>
 ...
 * O(L<sup>2</sup>).
 *
 * <p>See the following two publications for the original description of the VBSS algorithm:
 *
 * <ul>
...
public final class ValueBiasedStochasticSampling<T extends Copyable<T>>
   ...
   *
   * <p>The intended usage of this method is to provide a convenient way of constructing exponential
