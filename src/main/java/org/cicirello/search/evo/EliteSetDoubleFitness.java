 ...
 * fitness.
 */
final class EliteSetDoubleFitness<T extends Copyable<T>>
  ...
  EliteSetDoubleFitness(int numElite) {
   ...
   * package-private for use by classes in evo package for maintaining a set of elite population members.
   */
  EliteSetDoubleFitness(ArrayList<PopulationMember.DoubleFitness<T>> initialPop, int numElite) {
    this(numElite);
    offerAll(initialPop);
  }
