...
/**
 * JUnit tests for the various classes that implement hybrid mutation operators (i.e., that combine
...
public class HybridMutationTests {
  ...
  public void testConstructorExceptions() {
    ...
    IllegalArgumentException thrown =
        assertThrows(
            IllegalArgumentException.class, () -> new HybridMutation<TestObject>(mutators));
    thrown =
        assertThrows(
            IllegalArgumentException.class, () -> new HybridUndoableMutation<TestObject>(mutators));
    thrown =
        assertThrows(
  ...
  public void testWeightedHybridMutationEqualWeights() {
    ...
    for (int w = 1; w <= 2; w++) {
      for (int k = 1; k <= 3; k++) {
        ...
        WeightedHybridMutation<TestObject> m =
            new WeightedHybridMutation<TestObject>(mutators, weights);
        TestObject t = new TestObject();
        for (int i = 0; i < n; i++) {
          m.mutate(t);
        }
  ...
  public void testWeightedHybridUndoableMutationEqualWeights() {
    ...
    for (int w = 1; w <= 2; w++) {
      for (int k = 1; k <= 3; k++) {
        ...
        WeightedHybridUndoableMutation<TestObject> m =
            new WeightedHybridUndoableMutation<TestObject>(mutators, weights);
        TestObject t = new TestObject();
        // undo should do nothing if mutate not yet called
        // in this case, if it does something an assertion will fail within the undo
        m.undo(t);
  ...
  public void testWeightedHybridUndoableMutationUndoMethod() {
    int n = 100;
    int[][][] weights = {{{1}}, {{1, 1}, {1, 2}}, {{1, 1, 1}, {1, 2, 1}}};
    for (int k = 1; k <= 3; k++) {
      for (int w = 0; w < weights[k - 1].length; w++) {
        ...
        WeightedHybridUndoableMutation<TestObject> m =
            new WeightedHybridUndoableMutation<TestObject>(mutators, weights[k - 1][w]);
        TestObject t = new TestObject();
        for (int i = 0; i < n; i++) {
          m.mutate(t);
          m.mutate(t);
